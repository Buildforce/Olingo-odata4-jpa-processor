...
import nl.buildforce.sequoia.metadata.core.edm.annotation.EdmEnumeration.DummyConverter;
...
class IntermediateEnumerationType extends IntermediateModelElement implements JPAEnumerationAttribute {
    ...
    for (final Object constants : javaMembers) {
      if (constants instanceof Enum) {
        ...
        if (determineIsFlag() && value.longValue() < 0L)
          ...
          throw new ODataJPAModelException(ODataJPAModelException.MessageKeys.ENUMERATION_NO_NEGATIVE_VALUE, e.name(), javaEnum
              .getName());
  ...
  private <T extends Number> FullQualifiedName determineUnderlyingType() throws ODataJPAModelException {
    ...
    throw new ODataJPAModelException(ODataJPAModelException.MessageKeys.ENUMERATION_UNSUPPORTED_TYPE, javaEnum.getName(), type
