...
abstract class IntermediateStructuredType extends IntermediateModelElement implements JPAStructuredType {
  ...
  @Override
  public List<nl.buildforce.sequoia.metadata.core.edm.mapper.api.JPAProtectionInfo> getProtections() throws ODataJPAModelException {
...
/*          if (jpaAttribute.getJavaMember() instanceof AnnotatedElement) {
    ...
    if (protectedAttributes == null) {
      ...
      for (JPAAttribute attribute : getDeclaredAttributes()) {
        if (attribute.hasProtection()) {
            ...
            for (final String claimName : attribute.getProtectionClaimNames()) {
              for (final String pathName : attribute.getProtectionPath(claimName)) {
                ...
                if (path == null) // Annotation EdmProtectedBy found at '%2$s' of '%1$s', but the given 'path' '%3$s'...
                  throw new ODataJPAModelException(ODataJPAModelException.MessageKeys.COMPLEX_PROPERTY_WRONG_PROTECTION_PATH, attribute.getInternalName(),
          ...
          for (final nl.buildforce.sequoia.metadata.core.edm.mapper.api.JPAProtectionInfo info : attribute.getStructuredType().getProtections()) {
  ...
  private static class ProtectionInfo implements nl.buildforce.sequoia.metadata.core.edm.mapper.api.JPAProtectionInfo {
